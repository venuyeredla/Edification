...
solrAdminApp.controller('EnglishController',
  function($scope, $routeParams, $location, $http,Query, Constants){
    $scope.resetMenu("query", Constants.IS_COLLECTION_PAGE);

    // @todo read URL parameters into scope
    $scope.query = {wt: 'json', q:'*:*', indent:'off'};
    $scope.filters = [{fq:""}];
    ...
    $scope.doQuery = function() {
      ...
      Query.query(params, function(data) {
        ...
        $scope.response = angular.fromJson(data.data);
        $scope.url = $location.protocol() + "://" +
                     $location.host() + ":" +
                     $location.port() + url;
      });

    //  $scope.url='http://localhost:8983/solr/Dictionary/select?fl=word,meanings&indent=on&q=*:*&wt=json';
    };
